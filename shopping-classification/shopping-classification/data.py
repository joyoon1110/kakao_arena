...
class Data:
    y_vocab_path = 'D:/kakao_arena/shopping-classification/data/y_vocab.cPickle' if six.PY2 else 'D:/kakao_arena/shopping-classification/data/y_vocab.py3.cPickle'
    tmp_chunk_tpl = 'D:/kakao_arena/shopping-classification/tmp/base.chunk.%s'
    ...
    def build_y_vocab(self):
        ...
        try:
            rets = pool.map_async(build_y_vocab,
                                  [(data_path, 'train')
                                   for data_path in opt.train_data_list]).get(9999999)
            pool.close()
            pool.join()
            y_vocab = set()
    ...
    def make_db(self, data_name, output_dir='D:/kakao_arena/shopping-classification/data/train', train_ratio=0.8):
        if data_name == 'train':
            div = 'train'
